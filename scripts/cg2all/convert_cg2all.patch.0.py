...
def main():
    ...
    if config["cg_model"] == "CalphaBasedModel":
        ...
        #++++++++++++++++++++++++++++++
        class CalphaBasedModelCustom(cg2all.lib.libcg.CalphaBasedModel):
            def process(self, traj, pdb_fn, is_all=True, check_validity=True, compute_dssp=True, **kwarg):
                super().process(traj, pdb_fn, is_all, check_validity, compute_dssp, **kwarg)
                if hasattr(traj, "bfactors"):
                    pass
                else:
                    traj.bfactors = np.zeros((traj.xyz.shape[0], traj.xyz.shape[1]))
        cg_model = CalphaBasedModelCustom
        #++++++++++++++++++++++++++++++
    ...
    #     cg_model = cg2all.lib.libcg.MainchainModel
        ...
        raise KeyError(config["cg_model"])
    #
    ...
    if arg.in_dcd_fn is not None:
        #++++++++++++++++++++++++++++++
        _traj = mdtraj.load(arg.in_dcd_fn, top=arg.in_pdb_fn)
        input_s.n_frame = len(_traj)
        input_s.n_frame0 = input_s.n_frame+1
        #++++++++++++++++++++++++++++++
